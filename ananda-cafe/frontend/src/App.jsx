...
const OrderChallanView = ({ items, categories, displayCategory, selCat, setSelCat, orderQty, setOrderQty, setView, setDraft, fmt }) => {
  ...
  if (rmEditing) {
    return (<div>
      ...
      <div style={{ padding: "10px 14px", borderRadius: 10, background: "#EFF6FF", border: "1px solid #BFDBFE", fontSize: 12, color: "#1D4ED8", marginBottom: 14 }}>Set the quantity needed for 10 days of operation. Last 10 days actual usage shown as suggestion.</div>
      ...
      {items.filter(i => !selCat || displayCategory(i.category) === selCat).map((item) => {
        ...
        return (<div key={item.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderRadius: 10, background: rmDraft[item.id] > 0 ? "#EFF6FF" : "#FAFAF8", marginBottom: 3 }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 10, color: "#999" }}>Last 10d usage: <strong style={{ color: usage > 0 ? "#2563EB" : "#CCC" }}>{usage || "—"} {item.unit}</strong></div>
          </div>
    ...
    {Object.keys(rmConfig).length > 0 && (<>
      ...
      {orderItems.filter(i => !selCat || displayCategory(i.category) === selCat).map((item) => {
        ...
        return (<div key={item.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", borderRadius: 10, background: needsOrder ? "#EFF6FF" : "#FAFAF8", marginBottom: 3, border: needsOrder ? "1px solid #BFDBFE" : "1px solid transparent" }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 600 }}>{item.name}</div>
            <div style={{ fontSize: 10, color: "#999" }}>RM: <strong>{item.rmQty}</strong> − Stock: <strong style={{ color: Number(item.current_qty) === 0 ? "#DC2626" : "#888" }}>{item.current_qty}</strong> = <strong style={{ color: "#2563EB" }}>{item.orderQtyCalc}</strong> {item.unit}</div>
          </div>
