...
const DEMAND_SECTIONS = [
  { id: "food", titleHi: "Food (Prepared in BK)", emoji: "🍲", color: "#B45309", bg: "#FFFBEB", border: "#FDE68A",
    items: [
      ...
      { id: "boiled_rice", name: "Boiled Rice", unit: "Kg" },
      { id: "upma_sooji", name: "Upma Sooji", unit: "Kg" },
    ]},
  { id: "dairy", titleHi: "Dairy", emoji: "🥛", color: "#2563EB", bg: "#EFF6FF", border: "#BFDBFE",
    items: [
      { id: "butter", name: "Butter", unit: "Kg" },
      { id: "cheese", name: "Cheese", unit: "Kg" },
      { id: "paneer", name: "Paneer", unit: "Kg" },
      { id: "amul_cream", name: "Amul Cream", unit: "Kg" },
      { id: "dahi", name: "Dahi", unit: "Kg" },
  ...
  { id: "vegetable", titleHi: "Vegetables", emoji: "🥬", color: "#16A34A", bg: "#F0FDF4", border: "#BBF7D0",
    items: [
      ...
      { id: "green_chillies", name: "Green Chillies (Hari Mirch)", unit: "Kg" },
      { id: "coriander_leaves", name: "Coriander Leaves (Dhaniya Patta)", unit: "Kg" },
      { id: "curry_leaves", name: "Curry Leaves (Kari Patta)", unit: "Kg" },
      { id: "banana_leaves", name: "Banana Leaves", unit: "Pcs" },
      { id: "ginger", name: "Ginger", unit: "Kg" },
      { id: "coconut", name: "Coconut", unit: "Pcs" },
      { id: "carrot", name: "Carrot", unit: "Kg" },
      { id: "beans", name: "Beans", unit: "Kg" },
      { id: "potato", name: "Potato (Aloo)", unit: "Kg" },
      { id: "garlic", name: "Garlic", unit: "Kg" },
      { id: "mint", name: "Mint (Pudina)", unit: "Kg" },
  ...
  { id: "grocery", titleHi: "Grocery & Staples", emoji: "🛒", color: "#EA580C", bg: "#FFF7ED", border: "#FED7AA",
    items: [
      ...
      { id: "pista", name: "Pista", unit: "Kg" },
      { id: "almond", name: "Almond", unit: "Kg" },
      { id: "meetha_soda", name: "Meetha Soda", unit: "Kg" },
      { id: "dhaniya_whole", name: "Dhaniya Whole", unit: "Kg" },
      { id: "whole_red_chilli", name: "Whole Red Chilli", unit: "Kg" },
  ...
  { id: "masala", titleHi: "Masala & Spices", emoji: "🌶️", color: "#DC2626", bg: "#FEF2F2", border: "#FECACA",
    items: [
      ...
      { id: "jeera", name: "Jeera (Cumin)", unit: "Kg" },
      { id: "kali_mirch", name: "Kali Mirch (Black Pepper)", unit: "Kg" },
      { id: "red_chilli_powder", name: "Red Chilli Powder", unit: "Kg" },
      { id: "amchoor_powder", name: "Amchoor Powder", unit: "Kg" },
      { id: "sambhar_masala_777", name: "Sambhar Masala 777", unit: "Kg" },
      { id: "dhaniya_powder", name: "Dhaniya Powder", unit: "Kg" },
      { id: "haldi_powder", name: "Haldi Powder", unit: "Kg" },
      { id: "kitchen_king", name: "Kitchen King", unit: "Kg" },
      { id: "chat_masala", name: "Chat Masala", unit: "Kg" },
      { id: "kasturi_methi", name: "Kasturi Methi", unit: "Kg" },
      { id: "mustard_seeds", name: "Mustard Seeds (Rai)", unit: "Kg" },
      { id: "methi_dana", name: "Methi Dana", unit: "Kg" },
      { id: "ilaychi", name: "Hari Ilaychi", unit: "Kg" },
  ...
  { id: "packaging", titleHi: "Packaging & Disposal", emoji: "📦", color: "#2563EB", bg: "#EFF6FF", border: "#BFDBFE",
    items: [
      ...
      { id: "dosa_box_small", name: "Dosa Box Small", unit: "Pcs" },
      { id: "dosa_box_big", name: "Dosa Box Big", unit: "Pcs" },
      { id: "biopoly_16x20", name: "16*20 Biopolythene", unit: "Kg" },
      { id: "biopoly_13x16", name: "13*16 Biopolythene", unit: "Kg" },
      { id: "bio_garbage_bag", name: "Bio Garbage Bag Big Size", unit: "Pkt" },
      ...
      { id: "clean_wrap", name: "Clean Wrap", unit: "Pcs" },
      { id: "tissues", name: "Tissues", unit: "Bundle" },
...
const CLOSING_HIDDEN = new Set(["plain_idli", "butter_idli", "podi_idli", "dosa", "rawa_mix_item", "vada", "upma", "boiled_rice", "lemon_rice", "tomato_rice", "curd_rice", "roti", "sabji", "daal", "rice"]);
...
// ═════════════════════════════════════════════════════════════════════════════
// ═════════════════════════════════════════════════════════════════════════════
//  CASH LEDGER — per-outlet running credit/debit ledger. Outlet managers report
//  cash IN (daily sales) and expenses; owner/store manager independently record
//  cash OUT when they actually collect it — both write to the same
//  daily_outlet_sales.cash_deposited field, so there's one shared number instead
//  of two disconnected self-reports. Used by both the Owner and Store Manager
...
const CashLedger = () => {
  ...
  const loadMonth = useCallback(() => {
    if (!selOutlet) return;
    setLoading(true);
    api.getOutletSales({ outlet_id: selOutlet, from: `${selMonth}-01`, to: dates[dates.length - 1] || `${selMonth}-01` })
  ...
  const rowFor = (date) => rows.find((r) => r.date === date);
  const closingFor = (date) => {
    const r = rowFor(date);
    if (!r) return null;
    return Number(r.prev_day_cash || 0) + Number(r.cash_collected || 0) - Number(r.cash_expense || 0) - Number(r.cash_deposited || 0);
  };
  ...
  return (<div>
    <div style={{ marginBottom: 16 }}>
      <h3 style={{ fontSize: 16, fontWeight: 700, margin: "0 0 4px" }}>💵 Cash Ledger</h3>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(130px, 1fr))", gap: 8, marginBottom: 20 }}>
      {OWN_OUTLETS.map((o) => {
        ...
        return (
          <div key={o.id} onClick={() => setSelOutlet(o.id)} style={{ background: selOutlet === o.id ? "#1A1A1A" : "#fff", borderRadius: 10, border: "1px solid #E8E8E4", padding: "12px", cursor: "pointer", textAlign: "center" }}>
    ...
    {!loading && (
      <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #E8E8E4", overflow: "hidden" }}>
        <div style={{ padding: "10px 16px", background: "#F8F8F5", borderBottom: "1px solid #E8E8E4", fontSize: 13, fontWeight: 700 }}>{outletName}</div>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
            <thead><tr style={{ background: "#FAFAF8" }}>
              ...
              <th style={{ ...thS, textAlign: "right" }}>− Deposited</th>
              <th style={{ ...thS, textAlign: "left" }}>By</th>
              <th style={{ ...thS, textAlign: "right" }}>= Closing</th>
              {canRecord && <th style={thS}></th>}
            </tr></thead>
            <tbody>
              {dates.slice().reverse().map((d) => {
                const r = rowFor(d);
                const closing = closingFor(d);
                return (
                  <tr key={d} style={{ borderBottom: "1px solid #F0F0EC" }}>
                    <td style={tdS}>{d.slice(5)}</td>
                    <td style={{ ...tdS, textAlign: "right", fontFamily: "'JetBrains Mono'", color: "#888" }}>{r ? fmt(r.prev_day_cash) : "—"}</td>
                    <td style={{ ...tdS, textAlign: "right", fontFamily: "'JetBrains Mono'", color: "#16A34A" }}>{r ? fmt(r.cash_collected) : "—"}</td>
                    <td style={{ ...tdS, textAlign: "right", fontFamily: "'JetBrains Mono'", color: "#DC2626" }}>{r && r.cash_expense > 0 ? fmt(r.cash_expense) : "—"}</td>
                    <td style={{ ...tdS, textAlign: "right", fontFamily: "'JetBrains Mono'", color: "#2563EB" }}>{r && r.cash_deposited > 0 ? fmt(r.cash_deposited) : "—"}</td>
                    <td style={{ ...tdS, fontSize: 10, color: "#999" }}>{r?.cash_deposited_by || "—"}</td>
                    <td style={{ ...tdS, textAlign: "right", fontFamily: "'JetBrains Mono'", fontWeight: 700, color: closing > 0 ? "#B45309" : "#16A34A" }}>{closing != null ? fmt(closing) : "—"}</td>
                    {canRecord && (
                      <td style={tdS}>
                        <button onClick={() => openEdit(d)} style={{ padding: "3px 8px", borderRadius: 5, border: "1px solid #BFDBFE", background: "#EFF6FF", fontSize: 10, fontWeight: 700, color: "#2563EB", cursor: "pointer", fontFamily: "inherit" }}>✏️</button>
                      </td>
                    )}
                  </tr>
                );
...
// The 7 punches an outlet manager is expected to make every day, in the order they'd
// naturally come up during the day (morning demand first, dispatch verify once BK
// sends it, sales/wastage/closing/purchases at day's end). 'dispatch_verify' only ever
// shows up as missing when the backend actually found a dispatched-but-unreceived
// order — every other key is unconditional, checked once per outlet per day.
const PUNCH_TYPES = [
  { key: "demand", icon: "✏️", label: "Demand" },
  { key: "dispatch_verify", icon: "🚚", label: "Verify Dispatch Challan" },
  { key: "dairy_purchase", icon: "🥛", label: "Dairy Purchase" },
  { key: "cold_drink_purchase", icon: "🥤", label: "Cold Drink Purchase" },
...
const MissingPunches = ({ selOutlet }) => {
  ...
  const waText = () => {
    ...
    lines.push("Please punch these ASAP 🙏");
    return lines.join("\n");
  };
                ...
                return (
                  <div key={cat}>
                    ...
                    {isExpanded && sortedItems.map((item, i) => {
                      ...
                      return (
                        <div key={globalIdx} style={{ borderBottom: "1px solid #F5F5F3", background: "#fff" }}>
          ...
          return (<>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
              {[
                ...
                { l: "Margin", v: null, display: md.effective_sale > 0 ? (md.margin || 0) + "%" : "N/A", c: md.effective_sale > 0 ? ((md.margin || 0) >= 0 ? "#16A34A" : "#DC2626") : "#999", bg: "#fff", bc: "#E8E8E4" },
              ].map((s, i) => (
                <div key={i} style={{ flex: "1 1 100px", background: s.bg, borderRadius: 12, padding: "14px 16px", border: `1px solid ${s.bc}`, textAlign: "center" }}>
                  <div style={{ fontSize: 10, color: "#999", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.6, marginBottom: 4 }}>{s.l}</div>
                  <div style={{ fontSize: 20, fontWeight: 800, color: s.c, fontFamily: "'JetBrains Mono', monospace" }}>{s.display || fmt(s.v || 0)}</div>
...
const OrderChallanView = ({ items, categories, displayCategory, selCat, setSelCat, orderQty, setOrderQty, setView, setDraft, setPoMeta, setStockInPrices, fmt }) => {
  ...
  // Daily vendors (Vegetables/Dairy/Gas) ask Today-or-Tomorrow up front, before the
  // item list even opens — the Driver Portal matches Vegetable orders to the exact
  // calendar date, so forcing this choice at entry (instead of a header toggle easy
  // to miss) is what actually prevents an order silently never reaching the driver.
  ...
  if (pendingVendor) { const pv=ORDER_VENDORS.find(x=>x.id===pendingVendor); return (<div><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:24}}><BackBtn onClick={()=>setPendingVendor(null)} /><div style={{flex:1,fontSize:15,fontWeight:800}}>{pv?.label} Order</div></div>
    <div style={{textAlign:"center",marginBottom:28}}><div style={{fontSize:40,marginBottom:8}}>{pv?.label.split(" ")[0]}</div><div style={{fontSize:16,fontWeight:800}}>Ordering for today or tomorrow?</div><div style={{fontSize:12,color:"#888",marginTop:4}}>Pick the day this is meant to arrive — gets it right before anything else.</div></div>
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      <button onClick={()=>{setSelDate(today());setSelVendor(pendingVendor);setPendingVendor(null)}} style={{padding:"20px",borderRadius:16,border:"none",background:pv?.color||"#1A1A1A",color:"#fff",fontWeight:800,fontSize:17,cursor:"pointer",fontFamily:"inherit"}}>📅 Today <span style={{fontWeight:500,fontSize:12,opacity:0.85}}>({today()})</span></button>
      <button onClick={()=>{setSelDate(istDateAgo(-1));setSelVendor(pendingVendor);setPendingVendor(null)}} style={{padding:"20px",borderRadius:16,border:`2px solid ${pv?.border||"#E0E0DC"}`,background:pv?.bg||"#fff",color:pv?.color||"#1A1A1A",fontWeight:800,fontSize:17,cursor:"pointer",fontFamily:"inherit"}}>📅 Tomorrow <span style={{fontWeight:500,fontSize:12,opacity:0.85}}>({istDateAgo(-1)})</span></button>
    </div>
  </div>); }
  ...
  const newDraft={}; const newPrices={};
  ...
  if (view === "stock_in" || view === "stock_out") {
    ...
    return (<div>
      ...
      {(poMeta?.itemIds
        ...
        const poItem = poMeta?.items?.[item.id];
        return (<div key={item.id} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 10px", borderRadius: 10, background: isEdited ? "#FFFBEB" : draft[item.id] > 0 ? (isIn ? "#F0FDF4" : "#FEF2F2") : "#FAFAF8", marginBottom: 3, border: isEdited ? "1px solid #FDE68A" : "1px solid transparent" }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 10, color: "#999" }}>
              ...
              {poItem && <span> · Ordered: <strong style={{ color: "#1A1A1A" }}>{poItem.order_qty}</strong> {item.unit}</span>}
              {poItem?.bought_qty > 0 && <span> · Bought: <strong style={{ color: "#16A34A" }}>{poItem.bought_qty}</strong> {item.unit}{poItem.total_price > 0 ? ` for ₹${poItem.total_price}` : ""}</span>}
              {item.last_unit_price > 0 && <span style={{ color: "#888" }}> · ₹{Math.round(item.last_unit_price * 100) / 100}/{item.unit}</span>}
              {isIn && perUnit > 0 && <span style={{ color: "#16A34A", fontWeight: 600 }}> → ₹{perUnit}/{item.unit} received</span>}
            </div>
          </div>
...
const RecipesPanel = () => {
  ...
  // Reloads RECIPES/RAW_MATERIALS from the real backend tables (bk_recipes,
  // bk_recipe_ingredients, raw materials) after a write, and resyncs the local edit copy.
  const reloadFromServer = async () => {
    const [recipes] = await Promise.all([api.getMasterRecipes().catch(() => null)]);
  ...
  // ── Add a new raw material / ingredient, usable in any recipe ──
  const addNewItem = async () => {
    if (!newItemName.trim()) return;
    const id = newItemName.trim().toLowerCase().replace(/[^a-z0-9]+/g, "_");
    setSaving(true);
    try {
      await api.addRawMaterial({ id, name: newItemName.trim(), unit: newItemUnit });
      if (!RAW_MATERIALS.find((r) => r.id === id)) RAW_MATERIALS.push({ id, name: newItemName.trim(), unit: newItemUnit });
      setNewItemName(""); setNewItemUnit("Kg"); setAddingItem(false);
    } catch (e) { alert("Failed to add item: " + e.message); }
    finally { setSaving(false); }
  };

  // ── Create a brand-new recipe shell (no ingredients yet), then drop straight into Edit
  // mode on it so ingredients can be added right away via the existing add-ingredient flow ──
  const addNewRecipe = async () => {
    if (!newRecipeName.trim() || !newRecipeYieldQty) return;
    const id = newRecipeName.trim().toLowerCase().replace(/[^a-z0-9]+/g, "_");
    if (RECIPES[id]) { alert("A recipe with this name already exists"); return; }
    setSaving(true);
    try {
      const yieldLabel = `${newRecipeYieldQty} ${newRecipeYieldUnit}`;
      await api.saveRecipe({ id, name: newRecipeName.trim(), yield_qty: Number(newRecipeYieldQty), yield_unit: newRecipeYieldUnit, yield_label: yieldLabel, ingredients: [] });
      await reloadFromServer();
      setSel(id);
      setEditMode(true);
      setNewRecipeName(""); setNewRecipeYieldQty(""); setNewRecipeYieldUnit("Kg"); setAddingRecipe(false);
    } catch (e) { alert("Failed to add recipe: " + e.message); }
    finally { setSaving(false); }
  };
  ...
  const saveRecipes = async () => {
    const r = editRecipes[sel];
    setSaving(true);
    try {
      await api.saveRecipe({ id: sel, name: r.name, yield_qty: r.yieldQty, yield_unit: "Kg", yield_label: r.yield, ingredients: r.ingredients.map((i) => ({ rawId: i.rawId, qty: i.qty })) });
      await reloadFromServer();
      setSaved(true);
      setEditMode(false);
  ...
  return (
    <div id="print-recipes">
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          <button onClick={() => { setAddingItem(!addingItem); setAddingRecipe(false); }} style={{ padding: "6px 14px", borderRadius: 8, border: "1px solid #BBF7D0", background: addingItem ? "#16A34A" : "#F0FDF4", fontSize: 12, fontWeight: 600, color: addingItem ? "#fff" : "#16A34A", cursor: "pointer", fontFamily: "inherit" }}>{addingItem ? "✕ Cancel" : "+ Add Item"}</button>
          <button onClick={() => { setAddingRecipe(!addingRecipe); setAddingItem(false); }} style={{ padding: "6px 14px", borderRadius: 8, border: "1px solid #BFDBFE", background: addingRecipe ? "#2563EB" : "#EFF6FF", fontSize: 12, fontWeight: 600, color: addingRecipe ? "#fff" : "#2563EB", cursor: "pointer", fontFamily: "inherit" }}>{addingRecipe ? "✕ Cancel" : "+ Add Recipe"}</button>
          {!editMode ? (
            ...
            <>
              ...
              <button onClick={saveRecipes} disabled={saving} style={{ padding: "6px 14px", borderRadius: 8, border: "none", background: "#16A34A", fontSize: 12, fontWeight: 600, color: "#fff", cursor: saving ? "not-allowed" : "pointer", fontFamily: "inherit" }}>{saving ? "⏳ Saving..." : "💾 Save"}</button>
            </>
          )}
      ...
      {addingItem && (
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", padding: "10px 14px", borderRadius: 10, background: "#F0FDF4", border: "1px solid #BBF7D0", marginBottom: 14 }}>
          <input value={newItemName} onChange={(e) => setNewItemName(e.target.value)} placeholder="New item / raw material name" style={{ flex: 1, minWidth: 160, padding: "7px 10px", borderRadius: 6, border: "1px solid #E0E0DC", fontSize: 12, fontFamily: "inherit" }} />
          <select value={newItemUnit} onChange={(e) => setNewItemUnit(e.target.value)} style={{ width: 80, padding: "7px 4px", borderRadius: 6, border: "1px solid #E0E0DC", fontSize: 12, fontFamily: "inherit", background: "#fff", cursor: "pointer" }}>
            {UNITS.map((u) => <option key={u} value={u}>{u}</option>)}
          </select>
          <button onClick={addNewItem} disabled={saving || !newItemName.trim()} style={{ padding: "7px 14px", borderRadius: 6, border: "none", background: "#16A34A", color: "#fff", fontSize: 12, fontWeight: 700, cursor: saving ? "not-allowed" : "pointer" }}>{saving ? "⏳..." : "Add"}</button>
        </div>
      )}
      {addingRecipe && (
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", padding: "10px 14px", borderRadius: 10, background: "#EFF6FF", border: "1px solid #BFDBFE", marginBottom: 14 }}>
          <input value={newRecipeName} onChange={(e) => setNewRecipeName(e.target.value)} placeholder="New recipe name" style={{ flex: 1, minWidth: 160, padding: "7px 10px", borderRadius: 6, border: "1px solid #E0E0DC", fontSize: 12, fontFamily: "inherit" }} />
          <input value={newRecipeYieldQty} onChange={(e) => setNewRecipeYieldQty(e.target.value)} type="number" step="0.001" placeholder="Yield qty" style={{ width: 90, padding: "7px 8px", borderRadius: 6, border: "1px solid #E0E0DC", fontSize: 12, fontFamily: "inherit" }} />
          <select value={newRecipeYieldUnit} onChange={(e) => setNewRecipeYieldUnit(e.target.value)} style={{ width: 80, padding: "7px 4px", borderRadius: 6, border: "1px solid #E0E0DC", fontSize: 12, fontFamily: "inherit", background: "#fff", cursor: "pointer" }}>
            {UNITS.map((u) => <option key={u} value={u}>{u}</option>)}
          </select>
          <button onClick={addNewRecipe} disabled={saving || !newRecipeName.trim() || !newRecipeYieldQty} style={{ padding: "7px 14px", borderRadius: 6, border: "none", background: "#2563EB", color: "#fff", fontSize: 12, fontWeight: 700, cursor: saving ? "not-allowed" : "pointer" }}>{saving ? "⏳..." : "Add"}</button>
        </div>
      )}
      {saved && <div style={{ padding: "10px 14px", borderRadius: 10, background: "#F0FDF4", border: "1px solid #BBF7D0", fontSize: 12, color: "#166534", marginBottom: 14 }}>✅ Recipe saved to database.</div>}
...
// ═════════════════════════════════════════════════════════════════════════════
//  DEMAND HISTORY — View & Edit last 7 days of demands by outlet
// ═════════════════════════════════════════════════════════════════════════════
const DemandHistory = () => {
  ...
  const days = useMemo(() => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      result.push({ date: ds, label: i === 0 ? "Today" : i === 1 ? "Yesterday" : ds.slice(5) });
    }
    return result;
  }, []);
  ...
  useEffect(() => { fetchDemands(); }, [selOutlet]);

  // Group demands by date, then split AM/PM based on submitted_at
  ...
  const saveEdit = async () => {
    ...
    setEditSaving(true);
    try {
      await api.editItemQty(selOutlet, editItem.date, editItem.itemId, newQty, "demand_correction");
      setEditItem(null);
      await fetchDemands();
        ...
        {[{ id: "bk", short: "🏭 Base Kitchen" }, ...OUTLETS].map(o => (
          <button key={o.id} onClick={() => setSelOutlet(o.id)}
            style={{ padding: "7px 14px", borderRadius: 8, fontSize: 12, fontWeight: selOutlet === o.id ? 700 : 500,
              ...
              background: selOutlet === o.id ? "#1A1A1A" : "#fff", color: selOutlet === o.id ? "#fff" : "#888" }}>
            {o.short}
          </button>
        ))}
      </div>

      {loading && <div style={{ textAlign: "center", padding: 40, color: "#999" }}>⏳ Loading demands...</div>}

      {!loading && days.map(day => {
        const g = grouped[day.date] || { am: [], pm: [] };
        const totalAM = g.am.reduce((s, d) => s + Object.keys(d.items || {}).length, 0);
        ...
        return (
          <div key={day.date} style={{ marginBottom: 8, borderRadius: 12, border: "1px solid #E8E8E4", overflow: "hidden", background: "#fff" }}>
            ...
            <div onClick={() => setExpandedDay(isExpanded ? null : day.date)}
              style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", cursor: "pointer", background: isExpanded ? "#FAFAF8" : "#fff" }}>
              <div>
                <span style={{ fontSize: 14, fontWeight: 700 }}>{day.label}</span>
            ...
            {/* Expanded: AM/PM slots */}
            {isExpanded && (
              <div>
                {[{ key: "am", label: "🌅 Morning", data: g.am, color: "#B45309", bg: "#FFFBEB", border: "#FDE68A" },
                  { key: "pm", label: "🌙 PM", data: g.pm, color: "#2563EB", bg: "#EFF6FF", border: "#BFDBFE" }]
                            ...
                            {Object.entries(sections).map(([secName, secItems]) => (
                              <div key={secName}>
                                ...
                                {secItems.map(item => {
                                  ...
                                  if (isEditing) {
                                    return (
                                      <div key={item.id} style={{ padding: "8px 16px", background: "#FFFBEB", borderBottom: "1px solid #FDE68A" }}>
                                        <div style={{ fontSize: 11, fontWeight: 700, color: "#92400E", marginBottom: 6 }}>✏️ Edit {item.name}</div>
                                        <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 6 }}>
                                          <span style={{ fontSize: 10, color: "#999", minWidth: 60 }}>Current:</span>
                                          <span style={{ fontSize: 12, fontWeight: 700, fontFamily: "'JetBrains Mono'" }}>{item.qty} {item.unit}</span>
                                        </div>
                                        <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 8 }}>
                                          <span style={{ fontSize: 10, color: "#999", minWidth: 60 }}>New qty:</span>
                                          <input type="number" inputMode="decimal" step="any" autoFocus value={editItem.value}
                                            onChange={e => setEditItem({ ...editItem, value: e.target.value })}
                                            style={{ flex: 1, padding: "6px 8px", borderRadius: 6, border: "1px solid #E0E0DC", fontSize: 13, fontFamily: "'JetBrains Mono'", fontWeight: 700 }} />
                ...
                {g.am.length === 0 && g.pm.length === 0 && (
                  <div style={{ padding: "16px", textAlign: "center", color: "#999", fontSize: 12 }}>No demands submitted</div>
                )}
              </div>
            )}
          </div>
...
const FranchiseBilling = ({ lockedOutlet } = {}) => {
  ...
  const convMap = useMemo(() => {
    const m = {};
    Object.entries(conversions).forEach(([unitType, rows]) => { (rows || []).forEach((row) => { m[row.item_id] = { fromUnit: unitType, qty: Number(row.qty), baseUnit: row.base_unit }; }); });
    return m;
  }, [conversions]);

  // Demand unit -> rate card unit, chaining through unit_conversions then an SI step —
  // identical logic to Demand History / the backend P&L, so billed amounts agree everywhere.
  const getUnitConv = (demandUnit, rateUnit, itemId) => {
    const du = (demandUnit || '').toLowerCase();
    const ru = (rateUnit || '').toLowerCase();
    if (du === ru) return 1;
  ...
  const getDayQty = (itemId, date) => {
    const edited = (dayEdits[itemId] || {})[date];
    return edited !== undefined ? (Number(edited) || 0) : ((dayWise[itemId] || {})[date] || 0);
  };
  ...
  const downloadCSV = () => {
    if (viewMode === "daywise") {
      ...
      const rows = items.map((i) => {
        const perDay = days.map((ds) => Math.round(getDayQty(i.id, ds) * 100) / 100);
        const total = perDay.reduce((s, v) => s + v, 0);
        return [i.name, i.unit, ...perDay, Math.round(total * 100) / 100];
      });
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                  ...
                  <tbody>
                    {visibleItems.map((i) => {
                      ...
                      if (total === 0 && !hasAnyEdit) return null;
                      return (
                        <tr key={i.id} style={{ borderBottom: "1px solid #F0F0EC" }}>
                          ...
                          <td style={{ ...tdS, position: "sticky", left: 150, background: "#fff", zIndex: 1, textAlign: "right", fontFamily: "'JetBrains Mono'", fontWeight: 700, color: "#B45309", width: 80, minWidth: 80, boxShadow: "2px 0 4px rgba(0,0,0,0.04)" }}>{Math.round(total * 100) / 100}</td>
                          {days.map((ds, idx) => {
                            const v = perDay[idx];
    ...
    <div style={{ padding: "8px 16px 10px", background: "#FAFAF8", border: "1px solid #E8E8E4", borderRadius: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
        <span style={{ fontSize: 11, fontWeight: 700 }}>
          <span style={{ color: "#BBB", fontWeight: 500 }}>({item.used} {displayUnit} × ₹{displayRate})</span>
          {scCost != null && (
...
const SystemLogs = () => {
  ...
  const [selMonth, setSelMonth] = useState(() => { const d = istNow(); return `${d.getUTCFullYear()}-${String(d.getUTCMonth() + 1).padStart(2, "0")}`; });
  const [selCat, setSelCat] = useState(null); // null = all categories; else isolates just that one
  ...
  const filtered = logs.filter((l) => !selCat || l.category === selCat);
  const monthLabel = monthOptions.find((m) => m.value === selMonth)?.label || selMonth;
  ...
  return (
    <div>
      ...
      {/* Category filters — clicking a pill isolates that category (shows only it),
          click again (or "All") to go back to unfiltered */}
      <div style={{ display: "flex", gap: 6, marginBottom: 10, flexWrap: "wrap" }}>
        <button onClick={() => setSelCat(null)} style={{ padding: "6px 12px", borderRadius: 8, fontSize: 11, fontWeight: !selCat ? 700 : 500, border: !selCat ? "none" : "1px solid #E0E0DC", cursor: "pointer", fontFamily: "inherit", background: !selCat ? "#1A1A1A" : "#fff", color: !selCat ? "#fff" : "#888" }}>All</button>
        {LOG_CATEGORIES.map((c) => (
          <button key={c.id} onClick={() => setSelCat(selCat === c.id ? null : c.id)} style={{ padding: "6px 12px", borderRadius: 8, fontSize: 11, fontWeight: selCat === c.id ? 700 : 500, border: selCat === c.id ? "none" : "1px solid #E0E0DC", cursor: "pointer", fontFamily: "inherit", background: selCat === c.id ? c.bg : "#fff", color: selCat === c.id ? c.color : "#BBB", opacity: !selCat || selCat === c.id ? 1 : 0.6 }}>{c.label}</button>
        ))}
      </div>
...
const OutletMgr = ({ onBack }) => {
  ...
  if (screen === "wastage") { const ft = Object.values(draft).filter((v) => v > 0).length; const wastageSections = roleCategoryScope ? DEMAND_SECTIONS.filter((sec) => roleCategoryScope.includes(sec.id)) : DEMAND_SECTIONS;
    ...
    {wastageQuery ? (
      <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #E8E8E4", overflow: "hidden", marginBottom: 12 }}>
        <div style={{ padding: "6px 12px 12px" }}>
          {wastageSearchResults.length === 0 && <div style={{ padding: "20px", textAlign: "center", color: "#999", fontSize: 13 }}>No items match "{itemSearch}"</div>}
          {wastageSearchResults.map(wastageItemRow)}
        </div>
      </div>
    ) : (<>
    ...
    <div style={{ background: "#fff", borderRadius: 14, border: `1px solid ${activeSec.border}`, overflow: "hidden", marginBottom: 12 }}>
      ...
      <div style={{ padding: "6px 12px 12px" }}>{wastageFilterItems(activeSec.items).map(wastageItemRow)}</div>
    </div>
    </>)}
  ...
  if (screen === "close") {
    ...
    const csSearchResults = csQuery ? csSections.flatMap((s) => s.items).filter((i) => !CLOSING_HIDDEN.has(i.id)).map((i) => ({ id: `cs_${i.id}`, name: i.name, unit: i.unit })).filter((i) => i.name.toLowerCase().includes(csQuery)) : [];
    const csItemRow = (item, idx, arr) => { const isFilled = closing[item.id] !== undefined && closing[item.id] !== ""; const bareId = item.id.replace(/^cs_/, ""); return (
    ...
    return (<div><SavingOverlay />
      ...
      {csQuery ? (
        <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #E8E8E4", overflow: "hidden", marginBottom: 12 }}>
          {csSearchResults.length === 0 && <div style={{ padding: "20px", textAlign: "center", color: "#999", fontSize: 13 }}>No items match "{itemSearch}"</div>}
          {csSearchResults.map((item, idx) => csItemRow(item, idx, csSearchResults))}
        </div>
      ) : (<>
...
const QuickRecipeAdder = ({ unmatchedDishes, unmappedIngredients, dateStr, onSaved }) => {
  ...
  const saveQuickRecipe = async () => {
    ...
    const validRows = quickRecipe.rows.filter((r) => r.raw_material.trim() && Number(r.qty) > 0);
    if (!quickRecipe.category.trim() || validRows.length === 0) return;
    setSavingRecipe(true);
    try {
      const created = await api.createRecipe({ item_name: quickRecipe.dishName, category: quickRecipe.category.trim(), item_type: "Item" });
...
export default function AnandaCafe() {
  ...
  const [auditPinError, setAuditPinError] = useState("");
  const tryUnlockAudit = () => {
    if (auditPinInput === AUDIT_PIN) {
      setAuditUnlocked(true);
      try { sessionStorage.setItem("audit_unlocked", "1"); } catch (e) {}
      setAuditPinPrompt(false); setAuditPinInput(""); setAuditPinError("");
      setAuditDropdown(true); setBkDropdown(false);
    } else {
      setAuditPinError("Incorrect PIN");
    }
  };
  const [storeView, setStoreView] = useState("bk");
  ...
  if (!currentUser) return (<div style={PAGE}>{FONT}<div style={{ maxWidth: 400, margin: "0 auto", padding: "60px 20px" }}>
    <div style={{ textAlign: "center", marginBottom: 32 }}>
      <img src="/logo.png" alt="The Ananda Cafe" style={{ width: 64, height: 64, borderRadius: "50%", marginBottom: 8, objectFit: "cover" }} />
      <h1 style={{ fontSize: 26, fontWeight: 900, margin: "0 0 4px" }}>The Ananda Cafe</h1>
  ...
  if (effectiveApp === "owner") return (<div style={PAGE}>{FONT}
    ...
    {/* Audit PIN prompt */}
    {auditPinPrompt && (<>
      <div onClick={() => setAuditPinPrompt(false)} style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 998, background: "rgba(0,0,0,0.4)" }} />
      <div style={{ position: "fixed", top: "30%", left: "50%", transform: "translate(-50%, -50%)", background: "#fff", borderRadius: 14, border: "1px solid #E8E8E4", boxShadow: "0 12px 32px rgba(0,0,0,0.2)", zIndex: 999, width: 280, padding: "20px" }}>
        <div style={{ fontSize: 14, fontWeight: 800, marginBottom: 4 }}>🔒 Audit Section</div>
        <div style={{ fontSize: 11, color: "#888", marginBottom: 12 }}>Enter PIN to access sensitive owner data</div>
        <input type="password" inputMode="numeric" autoFocus value={auditPinInput}
          onChange={(e) => { setAuditPinInput(e.target.value); setAuditPinError(""); }}
          onKeyDown={(e) => { if (e.key === "Enter") tryUnlockAudit(); }}
          placeholder="PIN" style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: auditPinError ? "1px solid #DC2626" : "1px solid #E0E0DC", fontSize: 16, fontFamily: "'JetBrains Mono'", textAlign: "center", letterSpacing: 4, marginBottom: 8, boxSizing: "border-box" }} />
        {auditPinError && <div style={{ fontSize: 11, color: "#DC2626", marginBottom: 8 }}>{auditPinError}</div>}
        <div style={{ display: "flex", gap: 6 }}>
          <button onClick={tryUnlockAudit} style={{ flex: 1, padding: "9px", borderRadius: 8, border: "none", background: "#1A1A1A", color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Unlock</button>
          <button onClick={() => setAuditPinPrompt(false)} style={{ flex: 1, padding: "9px", borderRadius: 8, border: "1px solid #E0E0DC", background: "#fff", fontSize: 12, fontWeight: 600, color: "#888", cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
        </div>
      </div>
    </>)}
    <div style={{ maxWidth: ownerTab === "payroll" || ownerTab === "pnl" ? "100%" : 960, margin: "0 auto", padding: "20px 18px 40px" }}>
      ...
      {AUDIT_TABS.includes(ownerTab) && !auditUnlocked && (
        <div style={{ textAlign: "center", padding: 60 }}>
          ...
          <div style={{ fontSize: 14, color: "#888", marginBottom: 14 }}>This section is PIN-protected</div>
          <button onClick={() => { setAuditPinPrompt(true); setAuditPinInput(""); setAuditPinError(""); }} style={{ padding: "10px 20px", borderRadius: 10, border: "none", background: "#1A1A1A", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Enter PIN</button>
        </div>
      )}
